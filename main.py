...
sys.path.append("D:\Project\SensorSoftware\plot")
from draw_graph import DrawGraph
from PyQt5.QtWidgets import QApplication,QMainWindow,QWidget
from ui_window.plot_ui import Ui_Form
from PyQt5 import QtCore, QtGui, QtWidgets
from PyQt5.QtCore import QTimer
import serial.tools.list_ports
from ui_window.ui import Ui_MainWindow 
...
#        start_date = self.start_dateTimeEdit.dateTime()
#        print(start_date)
#        return start_date
#    
#    def read_end_date(self):
#        end_date = self.end_dateTimeEdit.dateTime()
#        print(end_date)
#        return end_date
...
#        self.select_time()
        values = self.__db_get.select_any_time()
        print(values)
       
    def select_time(self):
        if self.time_comboBox.currentIndex() == 0:
            values = self.__db_get.one_hour_data() 
            self.graph.plot_line(values)
            self.set_button()
      
        
        if self.time_comboBox.currentIndex() == 1:           
            values = self.__db_get.three_hour_data() 
            self.graph.plot_line(values)
            self.set_button()
